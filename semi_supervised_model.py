...
Run:
    python semi_supervised_model.py
"""
...
def resolve_data_file():
    """Find the dataset file, even if the file name has an extra .csv suffix."""
    candidate_names = [
        "iris_dataset.csv",
        "iris_dataset.csv.csv",
        "iris.csv",
        "iris_dataset.csv.csv.csv",
    ]
    ...
    matching_files = sorted(BASE_DIR.glob("*iris*.csv"))
    if matching_files:
        return matching_files[0]

    raise FileNotFoundError(
        f"Could not find the Iris dataset in {BASE_DIR}. "
        "Please ensure the CSV file is present in the project folder."
    )
